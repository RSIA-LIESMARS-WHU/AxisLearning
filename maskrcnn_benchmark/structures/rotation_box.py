...
class RBoxList(object):
    ...
    def __init__(self, bbox, image_size, mode="xywha"):
        ...
        if mode not in ("xywha", "4xy"):
            raise ValueError("mode should be 'xywha' or '4xy'")
    ...
    def convertToxywha(self, mode):
        ...
        if mode == "xywha":
            ...
            bbox = torch.cat(
                (x, y, shortW*2, longH*2, longTheta), dim=-1
            )
            # print(bbox)
    ...
    #     if mode == "4xy":
    #         x1, y1, x2, y2, x3, y3, x4, y4 = self.bbox.split(1, dim=-1)
    #         bbox = torch.cat((x1, y1, x2, y2, x3, y3, x4, y4), dim=-1)
    #         bbox = RBoxList(bbox, self.size, mode=mode)
    #     else:
    ...
    def rescale(self, boxes_margin, *args, **kwargs):
        ...
        # ratio = ratios[0]
        scaled_xc, scaled_yc, scaled_w, scaled_h, scaled_a = self._split_into_xywha_or_4xy()
        # scaled_xc *= ratio
        # scaled_yc *= ratio
    ...
    def resize(self, size, *args, **kwargs):
        ...
        if self.mode == "xywha":
            ...
            x1,y1, x2,y2, x3,y3, x4,y4 = self._split_into_xywha_or_4xy()
            x1 *= ratio
            y1 *= ratio
            x2 *= ratio
            y2 *= ratio
    ...
    def transpose4xy(self, method):
        ...
        if method == FLIP_LEFT_RIGHT:
            TO_REMOVE = 1
            x1 = image_width - x1 - TO_REMOVE
            x2 = image_width - x2 - TO_REMOVE
            x3 = image_width - x3 - TO_REMOVE
            x4 = image_width - x4 - TO_REMOVE
        ...
        bbox = RBoxList(transposed_boxes, self.size, mode="4xy")
        # bbox._copy_extra_fields(self)
        for k, v in self.extra_fields.items():
            if not isinstance(v, torch.Tensor):
    ...
    def _split_into_xywha_or_4xy(self):
