...
class FCOSLossComputation(object):
    ...
    # 1
    def __call__(self, locations, box_cls, box_regression, centerness, ignoreness, targets):
        """
        Arguments:
        ...
        for l in range(len(labels)):
            ...
            ignoreness_flatten.append(ignoreness[l].reshape[-1])
        # level batch*num_pos num_classes
        box_cls_flatten = torch.cat(box_cls_flatten, dim=0)
        box_regression_flatten = torch.cat(box_regression_flatten, dim=0)
        centerness_flatten = torch.cat(centerness_flatten, dim=0)
        ignoreness_flatten = torch.cat(ignoreness_flatten, dim=0)
        ...
        all_centerness_targets[ignore_inds] = 1 - all_centerness_targets[ignore_inds]
