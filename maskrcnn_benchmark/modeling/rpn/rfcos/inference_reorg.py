...
# add
from maskrcnn_benchmark.structures.rboxlist_ops import reorg_result
...
class RFCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        box_regression = box_regression.view(N, 5, H, W).permute(0, 2, 3, 1)
        box_regression = box_regression.reshape(N, -1, 5)
        centerness = centerness.view(N, 1, H, W).permute(0, 2, 3, 1)
        centerness = centerness.reshape(N, -1).sigmoid()

        # dx1 dy1 dx2 dy2  
        ori_PosClsCenter = torch.cat([box_regression[0,:,:4], box_cls[0], centerness[0].unsqueeze(-1)], dim=1)
        
        torch.cuda.empty_cache()
        reorg_boxcls_centerness = reorg_result(ori_PosClsCenter, H, W, C)
        torch.cuda.empty_cache()

        box_cls    = reorg_boxcls_centerness[:,:C].unsqueeze(0)
        centerness = reorg_boxcls_centerness[:, C].unsqueeze(0)
        ...
        for layer, (l, o, b, c) in enumerate(zip(locations, box_cls, box_regression, centerness)):
            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, b, c, image_sizes, normal_factor[layer]
                )
            )
