...
class RFCOSModule(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        # self.normal_factor = nn.ParameterList([nn.Parameter(torch.exp(torch.FloatTensor([2.7725])).cuda()),#16 
        #                                   nn.Parameter(torch.exp(torch.FloatTensor([3.4657])).cuda()),#32
        #                                   nn.Parameter(torch.exp(torch.FloatTensor([4.1588])).cuda()),#64
        #                                   nn.Parameter(torch.exp(torch.FloatTensor([4.852])).cuda()),#128
        #                                   nn.Parameter(torch.exp(torch.FloatTensor([5.545])).cuda()) ])
        
        self.normal_factor = nn.ParameterList([nn.Parameter(torch.FloatTensor([16]).cuda()),#16 
                                          nn.Parameter(torch.FloatTensor([32]).cuda()),#32
                                          nn.Parameter(torch.FloatTensor([64]).cuda()),#64
                                          nn.Parameter(torch.FloatTensor([128]).cuda()),#128
                                          nn.Parameter(torch.FloatTensor([256]).cuda()) ])

        # self.normal_factor[1].register_hook(print_grad)
        # normal_factor.register_forward_hook(for_hook)
        # print(self.normal_factor[1])
    ...
    def _forward_train(self, locations, box_cls, box_regression, centerness, targets):
        ...
        loss_box_cls, loss_box_reg, loss_centerness = self.loss_evaluator(
            locations, box_cls, box_regression, centerness, targets, self.normal_factor
        )
        losses = {
