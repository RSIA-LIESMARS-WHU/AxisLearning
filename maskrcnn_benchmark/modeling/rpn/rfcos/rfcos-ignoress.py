...
class RFCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        for i in range(2):
            ...
            #     )
            # )
            # share_tower.append(nn.GroupNorm(32, in_channels))
            # share_tower.append(nn.ReLU())
    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            # share_tower = self.share_tower(feature)
            share_tower = feature


            # 对FPN的结果进行卷积
            cls_tower = self.cls_tower(share_tower)
            logits.append(self.cls_logits(cls_tower))
            # print(logits[l].shape)

            centerness.append(self.centerness(cls_tower))
            ignoreness.append(self.ignoreness(cls_tower))
            # 每一层赋予一个权重
            # bbox_reg.append(torch.exp(self.scales[l](
            #     self.bbox_pred(self.bbox_tower(feature))
            # )))
            bbox_reg.append(self.bbox_pred(self.bbox_tower(share_tower)))
            # bbox_reg.append(self.bbox_pred(self.bbox_tower(feature)))
        return logits, bbox_reg, centerness, ignoreness
...
class RFCOSModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None):
        ...
        """
        box_cls, box_regression, centerness, ignoreness = self.head(features)
    ...
    def _forward_train(self, locations, box_cls, box_regression, centerness, ignoreness, targets):
        '''
        locations       不同分辨率 每个点中心xy坐标
        box_cls         logits
        box_regression  对logits使用torch.exp 
        '''
        loss_box_cls, loss_box_reg, loss_centerness, loss_ignoreness = self.loss_evaluator(
            locations, box_cls, box_regression, centerness, ignoreness, targets
        )
        losses = {
            "loss_cls": loss_box_cls,
            "loss_reg": loss_box_reg,
            "loss_centerness": loss_centerness,
            "loss_ignoreness": loss_ignoreness
        }
        return None, losses
