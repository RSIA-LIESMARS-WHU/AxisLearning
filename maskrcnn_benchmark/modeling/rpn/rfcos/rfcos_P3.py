...
from .loss_P3 import make_fcos_loss_evaluator
...
class RFCOSHead(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.attn_p3_2_p3 = nn.Conv2d(
            in_channels, 1, kernel_size=3, stride=1,
            padding=1
        )

        self.attn_p3_2_p4 = nn.Conv2d(
            in_channels, 1, kernel_size=3, stride=1,
            padding=1
        )
        self.attn_p3_2_p5 = nn.Conv2d(
            in_channels, 1, kernel_size=3, stride=1,
            padding=1
        )
        self.attn_p3_2_p6 = nn.Conv2d(
            in_channels, 1, kernel_size=3, stride=1,
            padding=1
        )
        self.attn_p3_2_p7 = nn.Conv2d(
            in_channels, 1, kernel_size=3, stride=1,
            padding=1
        )
        ...
        for modules in [self.cls_tower, self.bbox_tower,
                        ...
                        self.attn_p3_2_p3, self.attn_p3_2_p4, self.attn_p3_2_p5,
                        self.attn_p3_2_p6, self.attn_p3_2_p7]:
    ...
    def forward(self, x):
        ...
        P3 = x[0]
        p3_h, p3_w = P3.shape[2:]

        p3_muti_attn = P3 * self.attn_p3_2_p3(P3)
        p4_muti_attn = F.interpolate(x[1], size=[p3_h, p3_w], mode="bilinear") * self.attn_p3_2_p4(P3)
        p5_muti_attn = F.interpolate(x[2], size=[p3_h, p3_w], mode="bilinear") * self.attn_p3_2_p5(P3)
        p6_muti_attn = F.interpolate(x[3], size=[p3_h, p3_w], mode="bilinear") * self.attn_p3_2_p6(P3)
        p7_muti_attn = F.interpolate(x[4], size=[p3_h, p3_w], mode="bilinear") * self.attn_p3_2_p7(P3)
        x=[p3_muti_attn+p4_muti_attn+p5_muti_attn+p6_muti_attn+p7_muti_attn]
