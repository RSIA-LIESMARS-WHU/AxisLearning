...
class FCOSLossComputation(object):
    ...
    def __init__(self, cfg):
        ...
        self.centerness_loss_func = nn.BCEWithLogitsLoss()#reduce=False
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest, normal_factor):
        ...
        for im_i in range(len(targets)):
            ...
            reg_targets_per_im[:, -1][is_cared_in_the_level == 0]= 0#centerness -> 0
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        # centerness > -1 的位置回归正负类别
        valid_inds = torch.nonzero(reg_targets_flatten[:,-1] > -1).squeeze(1)
        # centerness 等于-1 置位0
        reg_targets_flatten[:,-1][reg_targets_flatten[:,-1] < 0] = 0
        ...
        #*******************************************# 
        centerness_weights = reg_targets_flatten[:, -1][valid_inds]
        #*******************************************# 
        ...
        # cls_weight = torch.where(all_centerness_weights==0, torch.full_like(all_centerness_weights, 0.8), all_centerness_weights).unsqueeze(-1)
        ...
        cls_weight = torch.where(centerness_weights==0, torch.full_like(centerness_weights, 0.8), centerness_weights).unsqueeze(-1)
        # 并不是所有点都是正样本
        cls_loss = 2*self.cls_loss_func(
