...
class RFCOSHead(torch.nn.Module):
    def __init__(self, cfg, backbone_out_channels, in_channels):
        ...
        self.cls_tower = nn.ModuleList(cls_tower)
        self.bbox_tower = nn.ModuleList(bbox_tower)
    ...
    def forward(self, backbone_features, fpn_features):
        ...
        for l, feature in enumerate(fpn_features):
            if l<3:
                # 对FPN的结果进行卷积
                cls_tower_2 = self.cls_tower[1](self.cls_tower[0](feature))
                box_tower_2 = self.bbox_tower[1](self.bbox_tower[0](feature))
                residual = self.residual_conv[l](backbone_features[l])
                cls_tower_4 = self.cls_tower[3](self.cls_tower[2](  cls_tower_2 + residual))
                box_tower_4 = self.bbox_tower[3](self.bbox_tower[2](box_tower_2 + residual))

                bbox_reg.append(self.bbox_pred(box_tower_4))
                
                logits.append(self.cls_logits(cls_tower_4))
                centerness.append(self.centerness(cls_tower_4))
            else:
                cls_tower = self.cls_tower[3](self.cls_tower[2](self.cls_tower[1](self.cls_tower[0](feature))))
                box_tower = self.bbox_tower[3](self.bbox_tower[2](self.bbox_tower[1](self.bbox_tower[0](feature))))
