...
__device__ inline void convert_region1(float * pts , float const * const region) {
...
__device__ inline float inter(float const * const region1, float const * const region2, bool* region2Ct_in_region1_flag) {
  ...
  *region2Ct_in_region1_flag = in_rect(region2[0], region2[1], pts1);
...
__device__ inline float devRotateIoU(float const * const region1, float const * const region2) {
  ...
  bool region2Ct_in_region1_flag = false;

  float area_inter = inter(region1, region2, &region2Ct_in_region1_flag);

  //return area_inter / area2;//max(area_inter / area1, area_inter / area2);

  // (1-abs(region2[4] - region1[4])/180.) *
  float IOU = area_inter / (area1 + area2 - area_inter);//
  // float selfiou = area_inter / area2 ;//
  // if (IOU < 0.2 && !region2Ct_in_region1_flag)
  //   return 0;
  // else if(region2Ct_in_region1_flag && selfiou>0.7)
  //   return 1;
  //   return IOU;
  // else
  return IOU;
