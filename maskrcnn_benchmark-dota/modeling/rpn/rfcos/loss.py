...
class FCOSLossComputation(object):
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest, normal_factor):
        ...
        for im_i in range(len(targets)):
            ...
            # sqrt(area)
            # max_reg_targets_per_im = torch.sqrt(reg_targets_per_im[:,1])
            ...
            # centerness = reg_targets_per_im[:10000, -1].reshape([100,100]).data.cpu().numpy()
            # print(centerness.min(),centerness.max())
            # plt.cla()
            # plt.clf()
            # sns.set()
            # ax = sns.heatmap(centerness, cmap='rainbow')
            # plt.show()
            ...
            reg_targets_per_im[:,2:7] = one_minusone * torch.abs(reg_targets_per_im[:,2:7])/normal_factor[1][:,None]#.cpu()#.cpu()

            labels.append(labels_per_im)
            # detax1 detay1  detax2 detay2 h centerness
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        # print(all_centerness_targets.max())
        # torch.sqrt(
        ...
        cls_loss = 2*self.cls_loss_func(
            box_cls_flatten,#.cpu()
            labels_flatten.int(),#.cpu()
            weight = cls_weight#torch.full_like(all_centerness_targets, 0.8).unsqueeze(-1)
        ) / (pos_inds.numel() + N)  # add N to avoid dividing by a zero
        # pos_cls_loss = self.cls_loss_func(
        #     box_cls_flatten[pos_inds],#.cpu()
        #     labels_flatten.int()[pos_inds],#.cpu()
        #     weight = all_centerness_targets[pos_inds]
        # ) / (pos_inds.numel() + N)  # add N to avoid dividing by a zero
        # neg_cls_loss = self.cls_loss_func(
        #     box_cls_flatten[1-pos_inds],#.cpu()
        #     labels_flatten.int()[1-pos_inds],#.cpu()
        # ) 
        # neg_cls_loss_sort, sort_index = neg_cls_loss.sort(descending=True)
        # neg_num = 3*pos_inds.numel()
        # neg_num = min(3*pos_inds.numel(), all_centerness_targets.numel() - pos_inds.numel())
        # neg_cls_loss = neg_cls_loss_sort[:neg_num] / (pos_inds.numel() + N)  # add N to avoid dividing by a zero
