...
class FCOSLossComputation(object):
    ...
    def __init__(self, cfg):
        ...
        deta2 = nn.Parameter(torch.FloatTensor([4.159]).cuda())#64+e^4.158=128

        start3 = nn.Parameter(torch.FloatTensor([4.852]).cuda())#e^4.852=128
        start3.register_hook(print_grad)
        deta3 = nn.Parameter(torch.FloatTensor([4.852]).cuda())#128+e^4.852=256

        start4 = nn.Parameter(torch.FloatTensor([5.545]).cuda())#e^5.545=256
        deta4 = nn.Parameter(torch.FloatTensor([5.545]).cuda())#256+e^5.545=512

        start5 = nn.Parameter(torch.FloatTensor([6.2383]).cuda())#e^6.2383=512
        ...
        #     [start5, deta5],
        # ]
        self.object_sizes_of_interest = nn.ParameterList([start1, deta1,start2, deta2,start3, deta3,start4, deta4,start5, deta5])
    # 2
    def prepare_targets(self, points, targets):
        ...
        for l, points_per_level in enumerate(points):
            size_small = torch.exp(self.object_sizes_of_interest[l*2])
            size_large = size_small + torch.exp(self.object_sizes_of_interest[l*2+1])
            # object_sizes_of_interest_l = [size_small, size_large]
            # print(self.object_sizes_of_interest[l*2], size_small)
            # 2
            object_sizes_of_interest_per_level = torch.cat( [size_small, size_large])#\
                # points_per_level.new_tensor(object_sizes_of_interest_l)
            # 1 2 -> len(points_per_level) 2
            object_sizes_of_interest_l = torch.unsqueeze(object_sizes_of_interest_per_level, 0).expand(len(points_per_level) , -1)
            
            expanded_object_sizes_of_interest.append(
               object_sizes_of_interest_l
            )
        ...
        # print(expanded_object_sizes_of_interest)
        ...
        for i in range(len(labels)):
            labels_imgs.append(torch.split(labels[i], num_points_per_level, dim=0)) 
            reg_targets_imgs.append(torch.split(reg_targets[i], num_points_per_level, dim=0))
        ...
        for level in range(len(points)):
            labels_level_first.append(
                torch.cat([labels_per_im[level] for labels_per_im in labels_imgs], dim=0)
            )
            reg_targets_level_first.append(
                torch.cat([reg_targets_per_im[level] for reg_targets_per_im in reg_targets_imgs], dim=0)
            )
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest, normal_factor):
        ...
        for im_i in range(len(targets)):
            ...
            mk = self.h(max_reg_targets_per_im - object_sizes_of_interest[:, 0]) - self.h(max_reg_targets_per_im-object_sizes_of_interest[:, 1])
            # print(max_reg_targets_per_im[0],max_reg_targets_per_im[1],max_reg_targets_per_im[2],max_reg_targets_per_im[3],max_reg_targets_per_im[4],max_reg_targets_per_im[5],max_reg_targets_per_im[6],max_reg_targets_per_im[7],max_reg_targets_per_im[8],max_reg_targets_per_im[9])
            mk = mk.unsqueeze(-1)
            # print(mk)
            ...
            pixel_labels_per_im = labels_per_im[reg_targets_per_im[:, 0].long()]

            # 落在目标框外面label为0
            pixel_labels_per_im[reg_targets_per_im[:, 1] < 0.5 ] = 0#bg
            # 感受野不够
            pixel_labels_per_im[is_cared_in_the_level == 0] = 0#bg
            ...
            # reg_targets_per_im[:,2:7
            pixel_reg_targets_per_im = reg_targets_per_im[:,2:7] * mk
            pixel_reg_targets_per_im_w_centerness = torch.cat([pixel_reg_targets_per_im, reg_targets_per_im[:,7:8]], dim=1)
            labels.append(pixel_labels_per_im)
            reg_targets.append(pixel_reg_targets_per_im_w_centerness)
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        if pos_inds.numel() > 0:
            ...
            centerness_targets = reg_targets_flatten[:, -1]
