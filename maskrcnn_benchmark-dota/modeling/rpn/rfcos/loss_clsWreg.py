...
class FCOSLossComputation(object):
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, targets):
        ...
        # torch.sqrt(
        # cls_weight=torch.where(all_centerness_targets==0, torch.ones_like(all_centerness_targets), torch.sqrt(all_centerness_targets)).unsqueeze(-1)
        
        # cls_weight=torch.where(all_centerness_targets==0, torch.full_like(all_centerness_targets, 0.4), all_centerness_targets).unsqueeze(-1)
